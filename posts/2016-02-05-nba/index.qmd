...
ggplot(dsum.home, aes(x = standing, y = med.price, label = team)) +
  geom_point(size = 3, color = "#1565c0") +
  geom_text(hjust = -0.1, vjust = 0.5, size = 2.5, check_overlap = TRUE) +
  labs(x = "Current Team Standing", y = "Ticket price, home games ($)") +
  theme_minimal() +
  scale_y_continuous(limits = c(25, 375))
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ggplot(dsum.home, aes(x = away.med.price, y = med.price, label = team)) +
  geom_point(size = 3, color = "#1565c0") +
  geom_text(hjust = -0.1, vjust = 0.5, size = 2.5, check_overlap = TRUE) +
  labs(x = "Ticket price, away games ($)", y = "Ticket price, home games ($)") +
  theme_minimal() +
  scale_y_continuous(limits = c(25, 375)) +
  scale_x_continuous(limits = c(25, 375))
```


### You pay for what you get

The plot below originally showed median ticket price for all remaining regular games in the 15/16 season, plotted against the average standing of the two teams playing, as an interactive Shiny app. The interactive app is no longer available, but the analysis code can be found [here](https://github.com/MJAlexander/nba).
...
#### Notes:
These charts were originally made using `rCharts` and have been converted to `ggplot2`. Code for scraping StubHub and analysis can be found [here](https://github.com/MJAlexander/nba).
