...
The Stan code for model 1 is below. You can view the stan code for model 2 [here](https://github.com/MJAlexander/applied-stats/blob/master/code/models/simple_weight_preterm_int.stan). It's worth noticing the `generated quantities` block: for our model checks later on, we need to generate 1) pointwise estimates of the log-likelihood and 2) samples from the posterior predictive distribution. When using `brms` we don't have to worry about this because the log-likelihood is calculated by default, and the posterior predictive samples can be generated afterwards.[^5]

[^5]: It's also possible / easy to get posterior predictive samples in R after running the model, [here](https://github.com/MJAlexander/applied-stats/blob/master/code/yrep_birthweight_example.R) is some example code. Doing everything in Stan is neater, but it might be clearer to see what's going on doing it yourself in R. 
